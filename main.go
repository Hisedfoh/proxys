...
var proxy = Proxy{hitbox: 0.6, damage: false, heal: false, fly: false, antikb: false, jumpboost: false, speed: false, killaura: false, haste: false, slowfalling: false, noclip: false, nightvision: false}
var application App
		...
		for {
			...
			case *packet.CommandRequest:
				...
				case "help":
					...
					sendMessage(conn, `§1§l => §r§9damage`)
				...
				case "s":
					if len(args) < 3 && len(args) > 1 {
						switch args[1] {
						case "on":
							_ = conn.WritePacket(&packet.RequestAbility{Ability: packet.AbilityWalkSpeed})
							sendMessage(conn, "§aFly activer§a !")
							continue
						case "off":
							_ = conn.WritePacket(&packet.SetPlayerGameType{GameType: packet.GameTypeSurvival})
							sendMessage(conn, "§a Fly desactiver§a!")
							continue
						case "speed":
							_ = conn.WritePacket(&packet.SetPlayerGameType{GameType: packet.GameTypeSurvival})
							sendMessage(conn, "§a vitesse fly activer §a!")

						default:
							sendMessage(conn, "§cUnknown \""+args[1]+"\" fly mode!")
							break
						}
					} else {
						sendMessage(conn, "§cUsage: "+PREFIX+"fly on/off/speed")
					}
					continue
				case "hitbox":
					if len(args) > 1 {
